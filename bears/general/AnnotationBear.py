...
class AnnotationBear(LocalBear):
    ...
    def find_annotation_ranges(self,
        ...
        while position <= len(text):
            ...
            def get_new_position():
                for string in multiline_string_delimiters.keys():
                    if text[position:].startswith(string):
                        _range, end_position = self.get_multiline(
                            ...
                            text,
                            string,
                            multiline_string_delimiters[string],
                            position)
                        strings_range.append(_range)
                        return end_position + 1

                for string in string_delimiters.keys():
                    if text[position:].startswith(string):
                ...
                for comment in multiline_comment_delimiters.keys():
                    ...
                    if text[position:].startswith(comment):
                        _range, end_position = self.get_multiline(
                            ...
                            text,
                            comment,
                            multiline_comment_delimiters[comment],
                            position)
                        comments_range.append(_range)
                        return end_position + 1

                for comment in comment_delimiter.keys():
                    if text[position:].startswith(comment):
    ...
    def get_singleline_comment(file, filename, text, comment, position):
        """
        Get Sourcerange of a single-line comment where the start is the
